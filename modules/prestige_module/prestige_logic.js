// /game/modules/prestige_module/prestige_logic.js (v4.9 - Completed Prestige Logic)
    ...
    for (const producerId in prestigeData.producers) {
        ...
        if (producerDef.effect) {
            ...
            coreUpgradeManager.registerEffectSource(
                'prestige', 
                `${producerId}_effect_multiplier`,
                producerDef.effect.targetSystem, 
                producerDef.effect.targetId, 
                producerDef.effect.type, 
                effectValueProvider
            );
            loggingSystem.debug("PrestigeLogic", `Registered/Updated ${producerId} effect.`);
        }
    }
};
    ...
    const confirmationMessage = `
        <div class="space-y-3 text-left text-textPrimary">
            ...
            <div class="p-3 bg-red-900 bg-opacity-50 rounded-lg border border-red-700">
             ...
             <p class="text-xs text-gray-400">Achievements, Unlocked Tabs, Prestige Upgrades, and Prestige Skills are kept.</p>
        </div>
    `;
    ...
    coreUIManager.showModal("Confirm Prestige", confirmationMessage, [
            {
                ...
                callback: () => {
                    // --- MODIFICATION: Full logic implementation ---
                    ...
                    loggingSystem.info("PrestigeLogic", "Performing Prestige.", {prestigeRecord, newSnapshot});
